...
export type ValidatedRepoInfo = {
  type: 'success';
  /** Which cli command name this repository should use, e.g. `git` */
  command: string;
  /**
  ...
  /**
   * Directory containing git internal information for this repo, usually `${repoRoot}/.git`.
   */
  dotdir: AbsolutePath;
...
export type PreferredSubmitCommand = 'pr' | 'ghstack' | 'push' | 'submit';
...
export enum CommandRunner {
  /** Run as `git` */
  Git = 'git',
  /** Run as `gt` (Graphite CLI) for stack-aware operations like sync and submit */
  Graphite = 'gt',
  /**
   * Use the configured Code Review provider to run this command,
   * such as a non-git external submit command
   */
