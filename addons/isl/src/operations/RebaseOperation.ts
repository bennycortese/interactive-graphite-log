...
export class RebaseOperation extends Operation {
  ...
  getArgs() {
    // git rebase --onto <dest> <src>^ moves src (and descendants) onto dest
    // The `<src>^` notation tells git to exclude the parent of src (i.e. rebase from src onwards)
    return ['rebase', '--onto', this.destination, this.source];
  }
