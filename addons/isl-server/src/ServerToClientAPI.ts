...
export default class ServerToClientAPI {
    ...
    switch (data.type) {
      case 'subscribe': {
        ...
        switch (kind) {
          ...
          case 'submodules': {
            // Submodules not supported - don't respond so atom keeps its initial new Map()
      ...
      case 'addDiffComment': {
        repo.codeReviewProvider
          ?.addComment?.(data.diffId, data.body)
          ?.then(() => {
            this.postMessage({type: 'addedDiffComment', diffId: data.diffId});
          })
          .catch(error => {
            this.postMessage({
              type: 'addedDiffComment',
              diffId: data.diffId,
              error: String(error),
            });
          });
      ...
      case 'exportStack': {
        // debugexportstack is Sapling-specific; not supported for git/graphite MVP
        this.postMessage({
          type: 'exportedStack',
          assumeTracked: data.assumeTracked ?? [],
          revs: data.revs,
          stack: [],
          error: 'exportStack is not supported for git repositories',
        });
        break;
      }
      case 'importStack': {
        // debugimportstack is Sapling-specific; not supported for git/graphite MVP
        this.postMessage({
          type: 'importedStack',
          imported: [],
          error: 'importStack is not supported for git repositories',
        });
        break;
      }
      case 'fetchQeFlag': {
      ...
      case 'fetchActiveAlerts': {
        // No active alerts in git/graphite mode
        Promise.resolve()
          .then(() => {})
          .catch(err => {
  ...
  private async handleFetchCommitMessageTemplate(repo: Repository, ctx: RepositoryContext) {
    const {logger} = ctx;
    try {
      // For git/graphite, use a simple default template rather than sl debugcommitmessage
      let template = '';

      // Allow Internal to provide a custom template if available
      const customTemplate = await Internal.getCustomDefaultCommitTemplate?.(
