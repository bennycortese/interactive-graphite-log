...
optional arguments:
  ...
  --kill           Do not start Interactive Graphite Log, just kill any previously running server on the specified port
                   Note that this will disrupt other windows still using the previous server.
  --force          Kill any existing server on the specified port, then start a new server.
                   Note that this will disrupt other windows still using the previous server.
  ...
  --platform       Set which platform implementation to use by changing the resulting URL.
                   Used to embed Interactive Graphite Log into non-browser web environments like IDEs.
...
export function parseArgs(args: Array<string> = process.argv.slice(2)): Args {
  ...
  let kill = false;
  let force = false;
  let command = process.env.GIT ?? process.env.GT ?? 'git';
...
export async function runProxyMain(args: Args) {
  ...
  if (force) {
    ...
    try {
      ...
      info(`killed Interactive Graphite Log server process ${foundPid}`);
    } catch (err: unknown) {
      info(`did not stop previous Interactive Graphite Log server: ${(err as Error).toString()}`);
    }
  } else if (kill) {
    let foundPid;
    try {
  ...
  if (result.type === 'addressInUse' && !force) {
    ...
    if (!existingServerInfo) {
      const errorMessage =
        'failed to find existing server file. This port might not be being used by a Interactive Graphite Log server.\n' +
    ...
    if (pid == null) {
      const errorMessage =
        `port ${port} is already in use, but not by an Interactive Graphite Log server.\n` +
        suggestDebugPortIssue(port);
      if (json) {
        outputJson({
          error: errorMessage,
    ...
    if (killAndSpawnAgain) {
      try {
        ...
        errorAndExit(`did not stop previous Interactive Graphite Log server: ${(err as Error).toString()}`);
      }
    ...
    info('re-used existing Interactive Graphite Log server');
    info('\naccess Interactive Graphite Log with this link:');
    info(String(url));
...
export async function killServerIfItExists(
  ...
  try {
    ...
    throw new Error(
      `could not kill previous Interactive Graphite Log server process with PID ${pid}. This instance may no longer be running.`,
    );
  }
  return pid;
}
